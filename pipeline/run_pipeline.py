...
Flow:
  1) ProcessingStep (preprocess.py): split + format data
  2) TrainingStep (JumpStart AutoGluon Tabular)
  3) ProcessingStep (evaluate.py): offline eval on val set
  4) ConditionStep (classification): accuracy >= threshold
  5) RegisterModel (only on pass)
"""
...
def get_pipeline(
    ...
    label_col = ParameterString(name="LabelColumn", default_value="label")

    # Kept for compatibility with your runner; we hard-code "classification" in steps below
    ...
    train_instance_type = ParameterString(name="TrainInstanceType", default_value="ml.g5.2xlarge")

    time_limit = ParameterInteger(name="TimeLimitSeconds", default_value=3600)
    auto_stack = ParameterString(name="AutoStack", default_value="True")
    ...
    step_process = ProcessingStep(
        ...
        job_arguments=[
            ...
            "--problem-type", "classification",       # classification-only
    ...
    estimator = Estimator(
        ...
        disable_profiler=True,  # optional: improves pipeline caching behavior
    )
    ...
    acc_json = JsonGet(
        ...
        json_path="primary_metric.value",
    )

    # Inference image for the registered model
    infer_image_uri = image_uris.retrieve(
        framework="autogluon",  # required in your env; ignored in newer SDKs
        region=region,
    ...
    # Build a concrete Model object for registration (fixes _current_job_name path)
    ...
    register_step_cls = RegisterModel(
        ...
        model=inference_model,                         # pass the Model object (robust across SDKs)
    ...
    pipeline = Pipeline(
        ...
        parameters=[
            ...
            val_fraction,
            acc_threshold,
            rmse_threshold,         # retained (unused) for compatibility with your run script
            model_package_group,
            approval_status,
        ],
        steps=[step_process, step_train, step_eval, cond_cls_ok],
        sagemaker_session=pipeline_sess,
    )
    return pipeline
